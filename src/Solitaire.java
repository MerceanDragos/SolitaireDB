...
public class Solitaire extends JFrame implements MouseListener {
    ...
    public void mouseReleased ( MouseEvent event ) {
        ...
        if ( event.getSource ( ) == controlPanels[0] ) {
            ...
            if ( playerID > 0 )
                SolitaireDataBase.updatePerformance ( playerID, false, board.getTimeDouble ( ), board.getMoves ( ) );

            board.newGame ( );

            updateAll ( );
        }
        else if ( event.getSource ( ) == controlPanels[2] ) {
            dispose ( );
            new LogInScreen ( true );
        }
        else if ( event.getSource ( ) == controlPanels[3] ) {
            new PerformanceFrame ( playerID );
        }
            ...
            if ( board.checkForWin ( ) ) {
                board.stopTime ( );
                if ( playerID > 0 )
                    SolitaireDataBase.updatePerformance ( playerID, true, board.getTimeDouble ( ), board.getMoves ( ) );
                winPopUp ( );
            }
            updateAll ( );
        }
    ...
    public Solitaire ( int newPlayerID ) {
        playerID = newPlayerID;

        if ( playerID > 0 ) {
            mainPanels[0].remove ( 2 );
            mainPanels[0].add ( controlPanels[3] );
        }
        else {
            mainPanels[0].remove ( 2 );
            mainPanels[0].add ( controlPanels[2] );
        }
